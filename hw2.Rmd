...
So the issue with finding the closest Denny's for a LaQuinta is that, that LaQuinta may not be the nearest LaQuinta to that Denny's. This creates a disrepancy. Instead, to conduct our distance analysis, we will calculate the distance to the nearest Denny's for all LaQuinta's and vice versa. We can then analyze our results to draw some conclusions about their locations. 
...
spatial_dist <- function(thisx,thisy,thatx,thaty){
  ...
  d = 2*R*asin(sqrt(a+b)) # haversine formula - distance in miles #
  return(5280*d) # distance between this and that in feet #
  # 1 mile = 5280 feet #
}
...
This result isn't surprising because Denny's is a popular restaurant that will draw in any customer that would like to eat. On the other hand, LaQuinta is a hotel, so it will only draw in customers who are travelling and need a place to stay. As a result, LaQuintas will, most likely, be found in towns or cities that have a lot of visitors and tourists, which is why they will be clustered.    
...
As you can see, we disagree with Hedberg's claim that "La Quinta is Spanish for next to Denny's". While it may occur in a way that presents itself as a common occurence, analysis of all locations has proven it to be not true. 